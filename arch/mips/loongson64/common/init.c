...
extern int ls7a_guest_pcibios_map_irq(const struct pci_dev *dev, u8 slot,
					u8 pin);
extern int ls7a_guest_pcibios_dev_init(struct pci_dev *dev);
...
static void loongson_pch_init(void)
{
	...
	case ACPI_IRQ_MODEL_IOAPIC:
		...
		if(cpu_guestmode){
			loongson_pch->pcibios_map_irq =
					ls7a_guest_pcibios_map_irq;
			loongson_pch->pcibios_dev_init =
					ls7a_guest_pcibios_dev_init;
		}
