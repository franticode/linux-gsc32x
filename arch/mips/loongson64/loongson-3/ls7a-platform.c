...
static struct pci_controller ls7a_guest_pci_controller = {
...
static void __init ls7a_arch_initcall(void)
{
	...
	if(cpu_guestmode){
		ls7a_guest_pci_controller.io_map_base = mips_io_port_base;
		if (acpi_disabled)
			register_pci_controller(&ls7a_guest_pci_controller);
	}else{
...
int ls7a_guest_pcibios_dev_init(struct pci_dev *dev)
{
	int irq = 0;
	...
	irq = ls7a_guest_pcibios_map_irq(dev, dev->devfn >> 3, 1);
	if (irq >= 0) {
		dev->irq = irq;
		dev->irq_managed = 1;
	}
...
void get_pci_root_info (struct pci_controller **controller, struct list_head *resources, bool is_use_crs)
{
	if(is_use_crs) {
		if (cpu_guestmode) {
			*controller = &ls7a_guest_pci_controller;
		} else {
			*controller = &ls7a_pci_controller;
		}
	} else {
