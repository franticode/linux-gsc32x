...
static void kvm_ls7a_ioapic_raise(struct kvm *kvm, unsigned long mask)
{
	...
	for_each_set_bit(i, &val, 64) {
		if ((state->intisr & (0x1ULL << i)) == 0) {
			...
			ls7a_update_read_page_long(s,
					LS7A_INT_STATUS_OFFSET, state->intisr);
	...
	if (state->intedge & mask) {
		...
		if (!!level) {
			if ((state->intirr & mask) == 0) {
				state->intirr |= mask;
				ls7a_update_read_page_long(s, 0x380,
							   state->intirr);
	...
	if (offset & (len -1 )) {
		...
		data = *(unsigned char *)val;
				...
				state->htmsi_vector[offset_tmp] =
							(uint8_t)(data & 0xff);
				ls7a_update_read_page_char(s, offset,
					       state->htmsi_vector[offset_tmp]);
			}
...
struct ls7a_kvm_ioapic *kvm_create_ls7a_ioapic(struct kvm *kvm)
{
	...
	s->read_page = alloc_pages(GFP_KERNEL | __GFP_RETRY_MAYFAIL, 0);
			...
			struct ls7a_ioapic_state *state)
{
	...
	ls7a_update_read_page_long(ls7a_ioapic, LS7A_INT_MASK_OFFSET,
							ioapic_state->int_mask);
	ls7a_update_read_page_long(ls7a_ioapic, LS7A_INT_EDGE_OFFSET,
							ioapic_state->intedge);
	ls7a_update_read_page_long(ls7a_ioapic, LS7A_INT_STATUS_OFFSET,
							ioapic_state->intisr);
	ls7a_update_read_page_long(ls7a_ioapic, LS7A_INT_POL_OFFSET,
						    ioapic_state->int_polarity);
	ls7a_update_read_page_long(ls7a_ioapic, LS7A_HTMSI_EN_OFFSET,
							ioapic_state->htmsi_en);
	for (i = 0;i < 64; i++) {
		ls7a_update_read_page_char(ls7a_ioapic,
		      LS7A_ROUTE_ENTRY_OFFSET + i,ioapic_state->route_entry[i]);
		ls7a_update_read_page_char(ls7a_ioapic,
		       LS7A_HTMSI_VEC_OFFSET + i,ioapic_state->htmsi_vector[i]);
	}
	ls7a_ioapic_lock(ls7a_ioapic, &flags);
