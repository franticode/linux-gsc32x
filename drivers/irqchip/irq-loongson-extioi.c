...
int extioi_vec_init(struct fwnode_handle **handle, int model)
{
	int err, parent_irq;
	...
	/* route all io interrupts to IP3 */
	parent_irq = LOONGSON_BRIDGE_IRQ;

	extioi_priv->domain_handle = irq_domain_alloc_fwnode(extioi_priv);
	if (!extioi_priv->domain_handle) {
	...
	/* Setup IRQ domain */
	extioi_priv->extioi_domain = irq_domain_create_linear(extioi_priv->domain_handle, VEC_COUNT,
	...
	irq_set_chained_handler_and_data(parent_irq,
						 extioi_irq_dispatch, extioi_priv);

#ifdef CONFIG_LOONGSON_PCH_MSI
	pch_msi_init(extioi_priv->extioi_domain, true, model);
#endif
	if (handle)
		*handle = extioi_priv->domain_handle;
	return 0;

iounmap_base:
	iounmap(extioi_priv->base);
	extioi_priv->domain_handle = NULL;
	kfree(extioi_priv);
